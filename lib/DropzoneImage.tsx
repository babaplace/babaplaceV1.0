...
const MultiImageDropzone = React.forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    return (
      <div>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-[repeat(2,1fr)] lg:grid-cols-[repeat(3,1fr)] xl:grid-cols-[repeat(4,1fr)]">
          {/* Images */}
          {value?.map(({ key, file, progress }, index) => (
            <div
              key={index}
            ...
            >
              ...
              {imageUrls[index] && !disabled && progress === "COMPLETE" && (
                ...
                >
                  <div className="flex h-5 w-5 gap-3 cursor-pointer items-center justify-center rounded-md border border-solid border-gray-500 bg-white transition-all duration-300 hover:h-6 hover:w-6 dark:border-gray-400 dark:bg-black">
                    <X
          ...
          {(!value || value.length < (dropzoneOptions?.maxFiles ?? 0)) && (
            ...
            >
              ...
              <div className="flex flex-col items-center justify-center text-xs text-gray-400">
                ...
                <div className="text-gray-400">Glisser & Deposer</div>
                <div className="mt-3">
...
function CircleProgress({ progress }: { progress: number }) {
  ...
  return (
    <div className="relative h-16 w-16 bg-black bg-opacity-40">
      <svg
      ...
      <div className="absolute  border top-0 left-0 flex h-full w-full items-center justify-center text-xs text-white">
        {Math.round(progress)}%
      </div>
    </div>
  );
